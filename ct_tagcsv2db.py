...
csvfile, encode = '汉化词条29142-utf8.csv', 'utf8'  # 待导入文件 文件编码
overwriteTname = True
...
if __name__ == '__main__':
    ...
    for i, row in df.iterrows():    # 遍历数据行
        ...
        if re.search(r'[a-zA-Z]{2}_[a-zA-Z(][a-zA-Z]', row['name']):   # 如果词条名符合 aa_aa
            ...
            name = row['name'].replace('\'', '\\\'')                    if (row['name']        and (str(row['name'])        != 'nan')) else ''   # 词条名
        ...
        if len(odata) > 0:
            ...
            if (not odata['contributor']) or (odata['contributor'] == contributor+','):
                ...
                sql = f'''UPDATE `ct_tags` SET `t_name` = '{t_name}', `desc` = '{desc}', `c_id` = {c_id}, `remarks` = '{remarks}', `update_time` = NOW() WHERE `id` = {odata['id']};'''
                write_log(f"正在更新词条 {odata['name']} 译名为 {row['t_name']}\n=>  sql: {sql}")
                db.execute(sql)
                update_cnt += 1
